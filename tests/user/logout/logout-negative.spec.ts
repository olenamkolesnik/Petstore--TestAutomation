...
test.describe('User Authentication - Logout - Negative Scenarios', () => {
  test('should return unauthorized when logging out without login', async ({ request }) => {
  ...
  test('should not allow logging out twice (expired session)', async ({ authorizedClient }) => {
  ...
  INVALID_TOKENS.forEach((token, i) => {
    test(`should fail logout with malformed session token [${i + 1}]`, async ({ request }) => {
  ...
  WRONG_METHODS.forEach((method) => {
    test(`should reject logout attempt using HTTP ${method}`, async ({ request }) => {
  ...
  MALFORMED_QUERIES.forEach((query, i) => {
    test(`should handle malformed logout query params [${i + 1}]`, async ({ request }) => {
