...
import { HTTP_STATUS, RESPONSE_MESSAGES } from '../../../src/api/constants/constants';
import { env } from '../../../src/api/utils/env';

test.describe('User Login API - negative scenarios', () => {
  const INVALID_CREDENTIALS = [
    { username: 'InvalidUser20251118', password: 'WrongPassword1111120251118' },
    { username: env.username, password: 'WrongPassword1111120251118' },
    { username: env.username.toUpperCase(), password: 'WrongPassword1111120251118',
    },
  ];
  INVALID_CREDENTIALS.forEach(({ username, password }, index) => {
    test(`should fail to log in user with invalid credentials: username = ${username}, password = ${password}`, async ({
      ...
      expect(response.body.message).toBe(RESPONSE_MESSAGES.INVALID_CREDENTIALS);
